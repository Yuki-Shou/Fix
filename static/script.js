...
function poLinkedPRChanged() {
  ...
  const linked = pos.find(p => String(p.linked_pr) === String(prId));
  if (linked) {
...
async function savePO() {
  ...
  const po = {
    ...
    linked_pr: document.getElementById('po-linked-pr')?.value || null,
    items
  };
...
async function editPO(id) {
  try {
    ...
    const sel = document.getElementById('po-linked-pr'); if (sel && po.linked_pr) sel.value = po.linked_pr;
