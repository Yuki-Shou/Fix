...
def po_list(request):
    ...
    if request.method == 'POST':
        ...
        po = PurchaseOrder.objects.create(
            ...
            linked_pr_id=payload.get('linked_pr') or None,
        ...
        for it in payload.get('items',[]):
            POItem.objects.create(
                ...
                del_date=it.get('del_date') or None,
            )
...
def po_detail(request, pk):
    ...
    if request.method in ('PUT', 'PATCH'):
        ...
        for field in ('no','date','linked_pr','dept','supplier','tin','address','contact_person','contact_number','total','prepared_by','checked_by','approved_by'):
            if field in payload:
                # for FK fields the payload contains an id, so set *_id attribute
                if field == 'linked_pr':
                    po.linked_pr_id = payload.get(field) or None
                else:
